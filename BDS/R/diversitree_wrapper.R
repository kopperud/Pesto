anc.state.prob.bisse <- function(phy, datafile, lambda, mu, eta) {
  ...
  lik = diversitree::make.bisse(phy, states, control = list("backend", "deSolve"), strict=FALSE)
  rate = pars[1]
  num_taxa = length(states)
  #lnl <- lik(pars,root=ROOT.GIVEN, root.p=c(0.5,0.5), condition.surv=TRUE, intermediates=TRUE)
  lnl <- diversitree::lik(pars,root=ROOT.FLAT, condition.surv=TRUE, intermediates=TRUE)
  ...
  anc_states = diversitree::asr.marginal(lik, pars)
